...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi.biWidth; j++)
        {
            ...
            //if white -> make it black (more contrast)
            ...
            //if red -> make it black (remove noise)
            ...
            // if not red or white (then the color, in which the hidden message should be) -> turn yellow (more contrast)
            else /*if (triple.rgbtGreen != 0xff)*/
            {
                new_triple.rgbtRed = 0xff;
                new_triple.rgbtGreen = 0xff;
                new_triple.rgbtBlue = 0x00;
                fwrite(&new_triple, sizeof(RGBTRIPLE), 1, outptr);
            }
        }
